...
-- nante
create view v_demande_conge_categorie as  select dc.*, ctg.id as id_categorie, ctg.nom, ctg.valeur from demande_conge dc join employe e on dc.id_employe=e.id join contrat ct on e.id_contrat=ct.id join categorie ctg on ctg.id=ct.id_categorie;
...
-- Presence ou absence
create view v_presence_tmp as select id_employe, case when count(*) = 0 then true else false end presence from demande_conge d where (now()::date >= d.debut and now()::date <= d.fin) and status = 5 group by id_employe;
